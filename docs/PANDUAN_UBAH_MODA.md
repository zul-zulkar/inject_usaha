...
5. Konfirmasi (kamu yang klik di mode manual)
6. Cari ulang kode yang sama → Mode harus sudah PAPI (DIUBAH_TERVERIFIKASI)
        │
        ▼
kode berikutnya di list
```
...
2. Skrip mencari ulang kode itu dan memeriksa kolom Mode sudah PAPI → `DIUBAH_TERVERIFIKASI`.

Kalau ingin batal, tekan **Esc**. Kode itu tercatat `BELUM_BERUBAH` dan batch berhenti.
...
**e. (Opsional) Otomatis.** Skrip ikut mengklik item dan tombol konfirmasi. Mode ini hanya bisa jalan
kalau di browser ini sudah ada minimal satu hasil `manual` yang `DIUBAH_TERVERIFIKASI`, dan kamu
harus mengetik `YA`:

```js
await ubahModa.jalankan({mode: "otomatis", limit: 5, sayaSudahMelihatDialog: true})
```
...
`lewatiSelesai: false` (proses ulang yang sudah tuntas), `jedaMin`/`jedaMaks` (ms, default 3000–7000).
