 ...
 *     python fasih_sm/hapus_ganda/hapus_ganda.py
 * -> hapus_ganda_console.siap.js (TARGET = grup dari audit gabungan). Panduan langkah demi
 * langkah: fasih_sm/hapus_ganda/README.md.
 *
 * ALUR
 * ----
...
(function () {
   ...
   *  403 di get-by-assignment-id DUA ARTI (2026-09-24):
   *  - body ADA isi (mis. "Invalid CSRF Token") -> sesi/CSRF ditolak -> Berhenti.
   *  - body KOSONG -> biasanya dokumen ini sudah dihapus / di luar akses. Sesi tetap
   *    DIBUKTIKAN lewat dokumen/tabel lain (sesiHidup): hidup -> ada:false (tanda HILANG),
   *    mati -> Berhenti (jangan sampai SEMUA dokumen jadi TIDAK_TERBACA diam-diam). */
  async function bacaDetail(id) {
    ...
    for (let ke = 0; ; ke++) {
      ...
      if (r.status === 403 && !(await sesiHidup(id))) {
        throw new Berhenti("SESI_DITOLAK", `detail HTTP 403 & dokumen/tabel lain juga tidak terbaca — login ulang fasih-sm (akun admin, XSRF-TOKEN segar)`);
      }
      if (r.status === 403) {
  ...
  log(`siap: ${TARGET.length} grup ganda. Mulai: hapusGanda.cek() (read-only). Tutorial: fasih_sm/hapus_ganda/README.md`);
})();
