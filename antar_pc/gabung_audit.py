...
1. Hentikan/selesaikan batch di semua PC, lalu salin `audit_log_gabungan.csv`
   tiap PC ke SATU folder dgn nama berbeda, mis. `audit/pc/pc1.csv`, `pc2.csv`, ...
   — atau satu SUBFOLDER per PC tanpa ganti nama (`audit/pc/pc2/audit_log_gabungan.csv`),
...
2. Lihat laporannya dulu (tidak menulis apa pun):
       python antar_pc/gabung_audit.py --sumber audit/pc
3. Kalau tidak ada peringatan bentrok, tulis hasil gabungannya:
       python antar_pc/gabung_audit.py --sumber audit/pc --tulis
...
4. Salin `audit/pc/hasil/audit_log_gabungan.csv` KE SEMUA PC (termasuk PC ini).
5. Cocokkan dgn server (per akun, READ-ONLY dulu, lihat antar_pc/README.md):
       python input_usaha/sinkron_list.py --sumber <sheet> \
...
POLA_GANDA_DIHAPUS = lokasi.POLA_GANDA_DIHAPUS
KOLOM_GANDA = [
...
def kelompok_status(status: str) -> str:
    ...
    input_usaha (jangan disalin ulang ke sini) supaya tidak berbeda arti."""
    s = (status or "").strip()
...
def peringkat_status(alias: str) -> int:
    ...
    Kembar JS: peringkatStatus() di fasih_sm/hapus_ganda/hapus_ganda_console.js."""
...
def baca_info_server(pola: list[str]) -> dict:
    ...
    info: dict = {}
    for p in pola:
        for f in (lokasi.cari(p) if any(c in p for c in "*?") else [Path(p)]):
            if not f.exists():
                continue
            try:
...
def ringkas_per_kunci(gabungan: list[dict], asal_per_kunci: dict,
    ...
    sama dgn input_usaha: status & dokumen dari baris TERAKHIR."""
...
# `audit/pc/audit_log_gabungan.csv` bisa jadi salah satu audit SUMBER. Subfolder
...
def kumpulkan_sumber(sumber: list[str]) -> list[Path]:
    ...
    beserta SUBFOLDER-nya (mis. `audit/pc/pc2/audit_log_gabungan.csv`), jadi satu
...
def baca_status_server(pola: list[str]) -> tuple[dict, dict, dict]:
    ...
    berkas: list[Path] = []
    for p in pola:
        berkas.extend(lokasi.cari(p) if any(c in p for c in "*?") else [Path(p)])
    for f in berkas:
        if not f.exists():
            continue
        try:
...
def cetak_daftar_ganda(ganda: list[dict], sudah_dihapus: set, ada_status_server: bool) -> None:
    ...
    if pola:
        ...
        for p, n in pola.most_common(8):
            print(f"      {n:>5}  {p}")
    print("  Hapus lewat Console admin fasih-sm: python fasih_sm/hapus_ganda/hapus_ganda.py (lihat fasih_sm/hapus_ganda/README.md)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if status_server:
        ...
        print("\n(status server kosong — jalankan input_usaha/sinkron_list.py dulu "
    ...
    for judul, isi, catatan in (
        ...
        ("dokumen tercatat di >1 akun", bentrok["akun_ganda"], "input_usaha akan melewati baris ini"),
    ...
    info_server = baca_info_server(args.list_json or [lokasi.POLA_LIST_API])
