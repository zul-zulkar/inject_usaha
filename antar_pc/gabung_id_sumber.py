...
Kenapa ada: input_usaha/jalankan.py menulis ID dokumen ke sheet sumber MILIK PC
...
1. Salin sheet sumber tiap PC ke folder yang SAMA dgn audit-nya, satu subfolder per PC
   (berkas disalin apa adanya, tanpa ganti nama):
       ...
       audit/pc/pc3/audit_log_gabungan.csv   audit/pc/pc3/input_usaha.xlsx
   Alat ini hanya mengambil berkas ber-kolom ID; gabung_audit hanya mengambil audit.
2. Laporan dulu (tidak menulis apa pun):
       python antar_pc/gabung_id_sumber.py --utama bahan/input_usaha.xlsx --sumber audit/pc
3. Tulis (-> audit/pc/hasil/input_usaha.xlsx):
...
5. Sebarkan audit/pc/hasil/audit_log_gabungan.csv (ke audit/) & input_usaha.xlsx (ke bahan/)
...
from antar_pc.gabung_audit import FOLDER_HASIL, di_folder_hasil, folder_hasil  # noqa: E402
...
def daftar_berkas(sumber: list[str], utama: Path, format_sumber: str = "standar") -> list[Path]:
    ...
    mis. `audit/pc/pc2/input_usaha.xlsx`). Sheet utama sendiri, cadangan .bak-*,
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--utama", required=True, help="sheet yang diisi (mis. bahan/input_usaha.xlsx)")
    ap.add_argument("--sumber", required=True, action="append",
                    help="salinan sheet PC lain: berkas atau folder (boleh diulang)")
    from input_usaha.mesin import opsi_format
    opsi_format(ap)
    ...
    if any(k.kategori in ("BEDA", "KONFLIK") for k in keputusan):
        print("BEDA/KONFLIK = satu baris punya >1 dokumen di server (ganda). Cek daftar_ganda.csv "
              "(gabung_audit) / fasih_sm/hapus_ganda/README.md, lalu isi selnya manual.")
