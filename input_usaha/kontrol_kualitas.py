...
yang SAMA PERSIS dgn `input_usaha/jalankan.py --cek` (`muat_sumber()` -> loader ->
...
Contoh (jalankan dari root proyek):
    python input_usaha/kontrol_kualitas.py --sumber input_usaha.xlsx
    python input_usaha/kontrol_kualitas.py --sumber bahan/input_usaha.xlsx
    python input_usaha/kontrol_kualitas.py --sumber bahan/input_usaha.xlsx \\
        --dari 2 --sampai 500 --per-ppl
Tutorial: input_usaha/README.md
"""
...
KELUARAN_PATH = lokasi.HASIL_INPUT / "kontrol_kualitas.xlsx"
FOLDER_PER_PPL = str(lokasi.HASIL_INPUT / "kontrol_kualitas_per_ppl")
...
SARAN_MASALAH: dict[str, str] = {
    ...
    "BARIS_CONTOH": "Ini baris contoh dari templat: hapus barisnya (kolom 'Uraian:' diawali CONTOH).",
    ...
    "ID_DOKUMEN_GANDA": "Satu ID dokumen tertulis di beberapa baris (salah salin?). Sisakan di baris pemiliknya "
                        "saja — cek lewat input_usaha/tulis_id_sumber.py.",
}
...
def periksa_sumber(sumber: str, format_sumber: str = "standar", *, mode_satu_subsls: bool = True,
    ...
    """Pemeriksaan offline (SAMA dgn jalankan.py --cek) + pemetaan ke sel/kategori.
...
def _lembar_ringkasan(ws, lap: Laporan) -> None:
    ...
    ws["A4"] = ("Aturan = pemeriksaan offline jalankan.py --cek saat ini (tidak ada aturan tambahan). "
    ...
    tabel("Status input baris (keputusan input_usaha)", ("Status", "Baris", "Arti"),
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        description="Kontrol kualitas sumber data input usaha (format standar / tahap 2) — offline, "
                    "memakai pemeriksaan yang sama dgn jalankan.py --cek.")
    ap.add_argument("--sumber", required=True, help="File .xlsx/.csv sumber (sama dgn --sumber input_usaha)")
    mg.opsi_format(ap)
    ...
    ap.add_argument("--koordinat", choices=("otomatis", "wajib", "kirim"), default=None,
                    help="Sama dgn input_usaha (bawaan: otomatis utk tahap2, wajib utk standar)")
    ...
    ap.add_argument("--kodepos", default="", help="Tahap 2: kodepos cadangan (sama dgn input_usaha)")
    ...
    try:
        ...
        if args.format == "tahap2" and ("tidak sesuai" in str(e) or "tidak ada di" in str(e)):
            print("   Kolom sheet harus sama dgn templates/input_usaha.xlsx (docs/FORMAT_INPUT_USAHA.md).",
                  file=sys.stderr)
        return 2
    if not lap.dipilih:
