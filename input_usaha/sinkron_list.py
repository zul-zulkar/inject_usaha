...
--tulis menambahkan ke audit (tidak pernah mengubah server):
  - DOKUMEN_DIBUAT + URL utk dokumen server yang belum tercatat  -> input_usaha
...
Contoh:
    python input_usaha/sinkron_list.py --sumber bahan/input_usaha.xlsx --akun-tunggal ppl.contoh@gmail.com --subsls-tunggal 5108010010000105
    (tambahkan --tulis setelah laporan ditinjau; --dari-json utk memakai hasil unduhan terakhir;
    --audit <folder> utk batch dgn audit sendiri). Laporan: input_usaha/hasil/sinkron_list.csv,
    daftar server: input_usaha/hasil/list_api_<akun>.json.
"""
...
LAPORAN_PATH = lokasi.HASIL_INPUT / "sinkron_list.csv"
...
def main() -> int:
    ...
    if args.tulis:
        ...
        if kunci_akun is None:
            print(f"❌ Akun {akun} sedang dipakai input_usaha — tunggu selesai sebelum --tulis.", file=sys.stderr)
            return 2
    try:
        if args.dari_json:
            ...
            simpan = lokasi.siapkan(Path(args.simpan_json or lokasi.HASIL_INPUT / f"list_api_{akun.replace('@', '_at_')}.json"))
        ...
        with lokasi.siapkan(LAPORAN_PATH).open("w", newline="", encoding="utf-8") as f:
