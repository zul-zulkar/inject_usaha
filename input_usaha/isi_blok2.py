"""
isi_blok2.py — Isi form fasih-web dari SATU baris sheet input usaha (dulu
input_usaha/isi_blok2.py): SE2026-L BLOK II, KETERANGAN PEMBERI JAWABAN, CATATAN.

Urutan & penjagaan field bersyarat SENGAJA begini (jangan dibalik):
  - pilih_umkm_sls diisi SEBELUM keberadaan_usaha (hilang permanen setelahnya);
  - 13b4 diisi SETELAH KBLI (baru dirender begitu kategori KBLI terpilih);
  - blok finansial baru dirender setelah rincian 25 di-blur.

Nilai diambil APA ADANYA dari baris (sudah dipetakan & diperiksa inti/tahap2_loader.py).
Default config dipakai HANYA utk rincian yang kolomnya kosong/tidak ada, dan setiap
pemakaiannya dikembalikan sbg daftar ASUMSI (kolom review_disarankan audit). Rincian
bersyarat yang WAJIB tapi tidak ada nilainya (13d/13e, 19b) -> BarisPerluManual, tidak ditebak.
"""
...
class BarisPerluManual(RuntimeError):
    ...
    mesin.py mencatatnya sbg SKIP_<kode> (isi manual), bukan ERROR."""
...
def read_kategori_lapangan_usaha(page: Page) -> str:
    """Baca field readonly '13.h Kategori Lapangan Usaha' (auto-terisi begitu KBLI
    dipilih) — SINYAL tambahan, bukan satu-satunya dasar keputusan (keberadaan field
    dideteksi lewat FasihWebSession.komponen_ada())."""
    try:
...
def fill_keterangan_pemberi_jawaban(sess: FasihWebSession):
    """Section KETERANGAN PEMBERI JAWABAN. dataKey terverifikasi:
      nama_info_list          combobox (textarea) -> NAMA_PEMBERI_INFORMASI
...
def fill_catatan(sess: FasihWebSession):
    """Section CATATAN. dataKey terverifikasi: `waktu_selesai` (tombol "Ambil Waktu")
    dan `catatan` (textarea, SENGAJA dibiarkan kosong — opsional)."""
...
def isi_13de(sess: FasihWebSession, row: GabunganRow, judul_kbli: str = "") -> None:
    ...
    fasih-web belum terpetakan -> dicari lewat label.
...
def fill_blok2_gabungan(sess: FasihWebSession, row: GabunganRow) -> list[str]:
    ...
    # 13b4 diisi SETELAH KBLI: baru ter-render kalau
    ...
    if sess.komponen_ada("izin_edar_bpom"):
        ...
        # 20c wajib di SEMUA cabang jawaban 20a.
    ...
    # server lambat perlu ditunggu (record 2566 & 2571).
    def _deteksi_varian():
