...
Sumber (semuanya berisi pasangan wilayah -> kodepos yang pernah diisi/terkirim):
  KODEPOS_BY_IDSUBSLS  di inti/config_lokal.py (selalu dipakai)
  --dari <file>        sheet lama yang punya kolom kodepos (format agenda, mis.
                       arsip/Agenda.xlsx; boleh berulang)

Cara lain yang lebih sederhana: beri kolom `kodepos` di sheet input usaha (opsional,
lihat docs/FORMAT_INPUT_USAHA.md) — nilainya menang atas daftar di config.
...
Contoh:
    python input_usaha/kodepos_desa.py --sumber bahan/input_usaha.xlsx --dari arsip/Agenda.xlsx
    python input_usaha/kodepos_desa.py --sumber bahan/input_usaha.xlsx --dari arsip/Agenda.xlsx --tulis
...
CONFIG_LOKAL = Path(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__)))) / "inti" / "config_lokal.py"
AWAL_BLOK = "# >>> KODEPOS_BY_DESA — dibangkitkan input_usaha/kodepos_desa.py (jangan diedit manual) >>>"
# Penanda sebelum 2026-09-25 (skrip dulu di input_tahap2/) — tetap dikenali & diganti.
AWAL_BLOK_LAMA = "# >>> KODEPOS_BY_DESA — dibangkitkan input_tahap2/kodepos_desa.py (jangan diedit manual) >>>"
AKHIR_BLOK = "# <<< KODEPOS_BY_DESA <<<"


def kumpulkan(sumber_standar: list[str]) -> dict[str, Counter]:
...
def tulis_config_lokal(peta: dict[str, str]) -> None:
    ...
    isi = isi.replace(AWAL_BLOK_LAMA, AWAL_BLOK)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--dari", action="append", default=[],
                    help="sheet lama (format agenda) yang punya kolom kodepos; boleh berulang")
    ap.add_argument("--tulis", action="store_true", help="tulis hasilnya ke inti/config_lokal.py")
    args = ap.parse_args(argv)

    dari = [f for f in args.dari if Path(f).exists()]
    print(f"Sumber kodepos: KODEPOS_BY_IDSUBSLS config" + (f" + {dari}" if dari else ""))
    suara = kumpulkan(dari)
