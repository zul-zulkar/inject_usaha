...
menghasilkan tampilan Excel yang sama ("5108060006000224" -> "5.10806E+15"):
  ...
  1. baris yang sama di audit UTUH (cadangan .bak-*, audit_pc/, --rujukan) -> dipakai utuh
...
def subsls_dari_catatan_wilayah(teks: str, minimal: int = 16) -> str:
    ...
    terbaca (main_gabungan mencatatnya sbg idsubsls_input), selain itu subsls run.
...
def kode_dari_list_api(pola: str = "list_api_*.json") -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--sheet", default=SHEET_BAWAAN if Path(SHEET_BAWAAN).exists() else "",
                    help=f"sheet sumber sbg cadangan kunci & idsubsls (default {SHEET_BAWAAN} kalau ada; "
                         "'' = tanpa sheet)")
    ap.add_argument("--format", choices=["standar", "tahap2"], default="tahap2", help="format --sheet")
