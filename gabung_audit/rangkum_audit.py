...
from gabung_audit.gabung_audit import baca_status_server  # noqa: E402
...
def main(argv=None):
    ...
    pola_json = [] if args.tanpa_server else (args.list_json or ["list_api_*.json"])
    status_server, galat_server, _nama = baca_status_server(pola_json) if pola_json else ({}, {}, {})
    berkas_json = [f for p in pola_json for f in (sorted(Path().glob(p)) if any(c in p for c in "*?")
