...
benar-benar membereskan tetap `main_gabungan.py`/`main_tahap2.py` (di situ semua
...
dulu di sheet, dan ada yang cuma bisa diberesi admin pusat.
    ...
    python gabung_audit/bersihkan_error.py --sumber Agenda.xlsx --sumber Agenda2.xlsx
...
KELUARAN_PATH = Path("./bersihkan_error.csv")
...
def perintah(format_sumber: str, sumber: str, akun: str, subsls: str, baris: list[int],
    ...
    skrip = ("input_tahap2/main_tahap2.py" if format_sumber == "tahap2"
             else "input_gabungan/main_gabungan.py")
    return (f"python {skrip} --sumber {sumber} --akun-tunggal {akun} --subsls-tunggal {subsls} "
            f"--baris {ringkas_baris(baris)} --lewati-selesai" + (" --submit" if submit else ""))
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--format", choices=["standar", "tahap2"], default="standar",
                    help="format utk --sumber (bawaan standar)")
    ...
    if any(r["tindakan"] == "SINKRON_DULU" for r in keluar):
        ...
        for akun in sorted({r["akun_login"] for r in keluar if r["tindakan"] == "SINKRON_DULU" and r["akun_login"]}):
            ...
            bendera = " --format tahap2" if args.sumber_tahap2 and not args.sumber else ""
            sheet = (args.sumber or args.sumber_tahap2 or ["<sheet>"])[0]
            print(f"python input_gabungan/sinkron_list.py{bendera} --sumber {sheet} "
    ...
    with Path(args.keluaran).open("w", newline="", encoding="utf-8") as f:
